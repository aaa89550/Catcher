    ...
    <div className="min-h-screen bg-cream-50">
      ...
      <div className="px-4">
        <div className="flex gap-8 pb-8">
          {/* 左側邊欄 */}
          <aside className="w-64 bg-white rounded-xl shadow-sm p-6 h-fit ml-16">
            {/* 搜尋框 */}
            <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      ...
                      className="px-3 py-1 text-xs border border-primary-200 rounded-full hover:bg-primary-50 transition-colors"
                    >
                      {preset.label}
                    </button>
                  ))}
          ...
          {/* 右側主要內容 */}
          <main className="flex-1 max-w-6xl mr-8">
            {/* 頁面統計與切換按鈕 */}
            <div className="flex justify-between items-center mb-6">
              <div>
                <p className="text-primary-600">找到 {filteredServices.length} 個{selectedCategory}服務</p>
              </div>
            ...
            {viewMode === 'card' ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {filteredServices.map((service) => (
                  ...
                  >
                    {/* 圖片區域 */}
                    <div className="w-full h-28 bg-primary-50 rounded-lg mb-3 overflow-hidden">
                      {service.image ? (
                        <img 
                          src={service.image} 
                          alt={service.title}
                    ...
                    <div className="flex flex-wrap gap-1 mb-2">
                      {service.tags.slice(0, 1).map((tag) => (
                        <span key={tag} className="px-2 py-1 text-xs bg-accent-100 text-accent-700 rounded-full">
                          {tag}
                        </span>
                      ))}
              ...
              <div className="space-y-4">
                {filteredServices.map((service) => (
                  <Link
                    key={service.id}
                    to={`/services/${encodeURIComponent(selectedCategory)}/${service.id}`}
                    className="bg-white border border-primary-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 p-6 cursor-pointer block"
                  >
                    <div className="flex gap-6">
                      {/* 圖片 */}
                      <div className="w-32 h-24 bg-primary-50 rounded-lg flex-shrink-0 overflow-hidden">
                        {service.image ? (
                          <img 
                            src={service.image} 
                            alt={service.title}
            ...
            {filteredServices.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-primary-700 mb-2">找不到符合條件的服務</h3>
                <p className="text-primary-500">請嘗試調整搜尋條件或篩選器</p>
              </div>
            )}
          </main>
        </div>
